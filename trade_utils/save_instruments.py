...
def save_instrument_eq():
    ...
    if conn is None or cursor is None:
        logger.error("Failed to get database connection for save_instrument_eq")
        return
    ...
    try:
        ...
        logger.error(f"Error in save_instrument_eq: {str(e)}", exc_info=True)
    finally:
        db.close_db_conn()
...
def save_instrument_idx():
    ...
    if conn is None or cursor is None:
        logger.error("Failed to get database connection for save_instrument_idx")
        return
    ...
    try:
        ...
        logger.error(f"Error in save_instrument_idx: {str(e)}", exc_info=True)
    finally:
        db.close_db_conn()
