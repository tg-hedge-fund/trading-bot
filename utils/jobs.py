...
def run_job_every_mon_fri(scheduler, time, fn, *args):
    scheduler.every().monday.at(time).do(fn, *args)
    scheduler.every().tuesday.at(time).do(fn, *args)
    scheduler.every().wednesday.at(time).do(fn, *args)
    scheduler.every().thursday.at(time).do(fn, *args)
    scheduler.every().friday.at(time).do(fn, *args)

def run_job_everyday(scheduler, time, fn, *args):
    logger.info("running everyday job")
    scheduler.every().day.at(time).do(fn, *args)
